...
Inter1 = 2; % At x = 0.5 nm
Inter2 = 12; % At x = 5.5 nm
...
Vd = transpose(linspace(0,1,11));
Ec_Ei = 0.561004; % E_c-E_i (eV)

Nx = 13; Deltax = 0.5e-9;
Ny = 241; Deltay = 0.5e-9; y_12 = 81; y_23 = 161; % 120nm version
x = Deltax*([0:Nx-1])';
y = Deltay*([0:Ny-1])';
Ndon = 2e23*ones(1,Ny);
Ndon(1,1:y_12) = 5e25;
...
for i = 1:1
    ...
    for iNewton = 1:10
        ...
        for iValley = 1:3
            ...
            coef_Sch = 2*Lx*Ly/(2*pi)*sqrt(mass(1)*mass(2))*m0/(hbar^2)*(k_B*T);
                ...
                scaledEz = diag(eigenvalues)/(-2*mass(3)*m0*(Deltax/hbar)^2); % Eigenenergy, J
    ...
    n_integrated_Sch(i,s) = sum(sum(n_Sch_tot),2);
    end
    n_integrated_Sch = n_integrated_Sch*0.5e-9;
    Id = n_integrated_Sch*mobility*q*Vd(s,1)/(120e-7)*6e-7*1e-4;    
    figure(1);
    plot(Vd,Id(i,:));
    hold all;
end
figure(1);
xlabel('Drain Voltage [V]'); ylabel('Drain Current [A]');
