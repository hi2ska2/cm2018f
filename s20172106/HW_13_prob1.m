...
%         Jaco(1,1) = 1.0;
%         
%         res(N,1) = phi(N,1) - thermal*log(Ndon(N,1)/ni) - V_applied ;
%         Jaco(N,N) = 1.0;
% 
% 
%         for ii = 2:N-1
...
for bb = 0:N_bias
    phi_J = phi_bias(:,bb+1);
    elec_J = elec_bias(:,bb+1);
    
    for ss = 2:N
        x_1 = ( phi_J(ss) - phi_J(ss-1) )./ thermal ;

        J(ss,bb+1) = -(q*D_n/Deltax)*( elec_J(ss,1)*(x_1/(exp(x_1)-1)) - elec_J(ss-1,1)* (-x_1/(exp(-x_1) -1)) );
    end
end
